 ...
 *
 * A fun, animated CLI dashboard showing project stats.
...
function getProjectAge(firstCommitDate: Date | null): string {
...
function buildActivityCalendar(commitDates: string[]): string {
...
function buildHourHeatmap(commitHours: string[]): { heatmap: string; peakHour: number } {
...
function buildGrowthGraph(totalLines: number): string {
...
function getCodingStyle(commitHours: string[]): string {
  const counts = new Array(24).fill(0);
  for (const hour of commitHours) {
    const h = parseInt(hour, 10);
    if (!isNaN(h)) counts[h]++;
...
function getTrophies(lines: number, commits: number): Trophy[] {
...
function getFunFacts(lines: number): string[] {
  const facts: string[] = [];
...
function getEncouragement(commits: number, lines: number, streak: number): string {
  const messages = [
...
async function render() {
  ...
  try {
    ...
    const trophyCol = 30;
    const trophy = (i: number) => trophies[i] ? `${trophies[i].emoji} ${trophies[i].label}` : '';
